...
<a href="#runtime_define" name="runtime_define">#</a> <i>runtime</i>.<b>define</b>(<i>cell, definition</i>)

As a high level API, the [runtime](#runtimes) also offers [define](#runtime_define), which allows you to wire up a complete notebook cell in the runtime’s main module.

From the runtime’s perspective, the `cell` is an object that will maintain a record of connections and values, in `_variable`, `_source` and `_imports` properties.

The `definition` is an object generated by the **notebook-compiler**, containing: a cell `body` which is the string or function that defines the value of the cell, or a list of `modules` defining imported values from other notebooks, an `id`, a `name`, a list of `inputs`, and optionally a `viewof` or `mutable` flag.

```js
runtime.define(cell, {
  body: '"use strict";(function(md){return md`Hi`})',
  inputs: ["md"],
  name: "greeting",
  mutable: false,
  view: false
...
<a href="#runtime_delete" name="runtime_delete">#</a> <i>runtime</i>.<b>delete</b>(<i>cell</i>)

As the counterpart to [define](#runtime_define), [delete](#runtime_delete) removes a cell from the runtime. This disposes of and disconnects the cell’s runtime `variable`, it’s `source` (if it has one), and also cleans up its `inputs` — if it’s the final cell that references them.
