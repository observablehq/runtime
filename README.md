...
Defining a cell replaces the cell’s definition with a new function body, adding, removing, and redefining the cell’s *variable*, and *inputs*, and perhaps *source*, and wiring the changed variables into the [runtime’s](#runtimes) execution graph.

The `definition` is generally an object generated by the **notebook-compiler**, containing: a cell `body` which is the string or function that defines the value of the cell, or a list of `modules` defining imported values from other notebooks, a `name`, a list of `inputs`, and optionally a `viewof` or `mutable` flag.
